...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate RS/FSS for PetImages teacher and student models.")
    ...
    parser.add_argument(
        "--json-output",
        default=str(Path(__file__).resolve().parent / "distance" / "comparingModelsPetImagesNewNoPre" / "CAM1.json"),
    )
    parser.add_argument(
        "--text-output",
        default=str(Path(__file__).resolve().parent / "distance" / "comparingModelsPetImagesNewNoPre" / "CAM1.txt"),
    )
