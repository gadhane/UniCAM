...
def deprocess_image(img):
    # Ensure img is in float format to avoid overflow or underflow issues
    img = img.astype('float32')

    # Optional: Convert from BGR to RGB if your images are from OpenCV
    # img = img[..., ::-1]  # Uncomment if needed

    # Normalize the image to 0 - 1 range
    img -= img.min()  # Shift the minimum value to 0.
    img /= (img.max() - img.min() + 1e-5)  # Scale the values to the [0, 1] range.
