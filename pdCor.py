...
def P_Distance_Matrix(latent):
    n = latent.shape[0]
    matrix_a = torch.sqrt(
        torch.sum(torch.square(latent.unsqueeze(0) - latent.unsqueeze(1)), dim=-1) + 1e-18
    )
    matrix_A = matrix_a - torch.sum(matrix_a, dim=0, keepdims=True) / (n - 2) \
               - torch.sum(matrix_a, dim=1,keepdims=True) / (n - 2) \
...
def P_removal(matrix_A, matrix_C):
    denominator = bracket_op(matrix_C, matrix_C)
    safe_denominator = denominator + 1e-12
    result = matrix_A - bracket_op(matrix_A, matrix_C) / safe_denominator * matrix_C
    return result
