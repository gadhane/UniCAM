...
model4 = models.resnet34(pretrained=False)
...
if __name__ == '__main__':
    ...
    target_layers_TS1 = [modelTS1remain.modelX.layer3[-3]]
    target_layers_TS2 = [modelTS2remain.modelX.layer3[-3]]
    target_layers_TS3 = [modelTS3remain.modelX.layer3[-3]]

    target_layers_S1 = [modelS1remain.modelX.layer3[-1]]
    target_layers_S2 = [modelS2remain.modelX.layer3[-1]]
    target_layers_S3 = [modelS3remain.modelX.layer3[-1]]
    # target_layers_S3 = [modelS3remain.modelX.features[-1]]

    target_T = [model1.layer3[-3]]
    target_S1 = [model2.layer3[-1]]
    target_S2 = [model3.layer3[-1]]
    target_S3 = [model4.layer3[-1]]
    ...
    for batch_idx, (inputs, targets) in enumerate(test_loader):
        ...
        cdir = 'RemoveResults/GradCAMs/Case12_layer2/'
        ...
        for idx in range(len(targets)):
            ...
            with cam_algorithm(model=model1,
                ...
                _, visualizations = cam_metric(input_T.to(device), grayscale_cam, targets, model1, return_visualization=True)
                ...
                cv2.imwrite(str(cdir) + str(batch_idx) + str(idx) + f'_OrigT_visconc.jpg', visualization)
            ...
            with cam_algorithm(model=model2,
                ...
                # ===================================
                cam_metric = ROADLeastRelevantFirst(percentile=50)
                _, visualizations = cam_metric(input_T.to(device), grayscale_cam, targets, model2,
                ...
                cv2.imwrite(str(cdir) + str(batch_idx) + str(idx) + f'_OrigS1_visconc.jpg', visualization)
            ...
            with cam_algorithm(model=model3,
                ...
                # ===================================
                cam_metric = ROADLeastRelevantFirst(percentile=50)
                _, visualizations = cam_metric(input_T.to(device), grayscale_cam, targets, model3,
                                               return_visualization=True)
                visualization = visualizations[0].cpu().numpy().transpose((1, 2, 0))
                visualization = deprocess_image(visualization)
                # ===================================
                ...
                cv2.imwrite(str(cdir) + str(batch_idx) + str(idx) + f'_OrigS2_visconc.jpg', visualization)
            ...
            with cam_algorithm(model=model4,
                ...
                # ===================================
                cam_metric = ROADLeastRelevantFirst(percentile=50)
                _, visualizations = cam_metric(input_T.to(device), grayscale_cam, targets, model4,
                                               return_visualization=True)
                visualization = visualizations[0].cpu().numpy().transpose((1, 2, 0))
                visualization = deprocess_image(visualization)
                # ===================================
                ...
                cv2.imwrite(str(cdir) + str(batch_idx) + str(idx) + f'_OrigS3_visconc.jpg', visualization)
            ...
            # ===================================
            cam_metric = ROADLeastRelevantFirst(percentile=50)
            htmap = np.asarray(grayscale_cam_TS2[idx:idx + 1, ...])
            _, visualizations = cam_metric(input_T.to(device), htmap, targets, model1,
                                           return_visualization=True)
            visualization = visualizations[0].cpu().numpy().transpose((1, 2, 0))
            visualization = deprocess_image(visualization)
            cv2.imwrite(str(cdir) + str(batch_idx) + str(idx) + f'_TS2_visconc.jpg', visualization)
            ...
            # ===================================
            cam_metric = ROADLeastRelevantFirst(percentile=50)
            htmap = np.asarray(grayscale_cam_TS3[idx:idx + 1, ...])
            _, visualizations = cam_metric(input_T.to(device), htmap, targets, model1,
                                           return_visualization=True)
            visualization = visualizations[0].cpu().numpy().transpose((1, 2, 0))
            visualization = deprocess_image(visualization)
            cv2.imwrite(str(cdir) + str(batch_idx) + str(idx) + f'_TS3_visconc.jpg', visualization)
            ...
            # ===================================
            cam_metric = ROADLeastRelevantFirst(percentile=50)
            htmap = np.asarray(grayscale_cam_S2[idx:idx + 1, ...])
            _, visualizations = cam_metric(input_T.to(device), htmap, targets, model3,
                                           return_visualization=True)
            visualization = visualizations[0].cpu().numpy().transpose((1, 2, 0))
            visualization = deprocess_image(visualization)
            cv2.imwrite(str(cdir) + str(batch_idx) + str(idx) + f'_S2T_visconc.jpg', visualization)
            ...
            # ===================================
            cam_metric = ROADLeastRelevantFirst(percentile=50)
            htmap = np.asarray(grayscale_cam_S3[idx:idx + 1, ...])
            _, visualizations = cam_metric(input_T.to(device), htmap, targets, model4,
                                           return_visualization=True)
            visualization = visualizations[0].cpu().numpy().transpose((1, 2, 0))
            visualization = deprocess_image(visualization)
            cv2.imwrite(str(cdir) + str(batch_idx) + str(idx) + f'_S3T_visconc.jpg', visualization)
