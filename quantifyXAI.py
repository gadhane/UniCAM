...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Quantify XAI transfer metrics for the PetImages mixed-architecture experiment."
    )
    ...
    parser.add_argument(
        "--json-output",
        default=str(Path(__file__).resolve().parent / "distance" / "DiffArchoutput3.json"),
    )
    parser.add_argument(
        "--text-output",
        default=str(Path(__file__).resolve().parent / "distance" / "DiffArchoutput3.txt"),
    )
    ...
    report = evaluate_rsfss(
        model_specs=[
            ModelSpec(
                ...
                checkpoint="teacher_ResNet101.pt",
            ),
            ModelSpec(
                name="student_r50",
                architecture="resnet50",
                checkpoint="Vanilla_Resnet50_ResNet101.pt",
            ),
            ModelSpec(
                name="student_r18_r101",
                architecture="resnet18",
                checkpoint="Vanilla_Resnet18_ResNet101.pt",
            ),
            ModelSpec(
                name="student_r18_r50",
                architecture="resnet18",
                checkpoint="Vanilla_Resnet18_ITAResNet50_ResNet101.pt",
            ),
        ],
